...
The phone is a remote interface for the Council running on the computer. Model requests still originate from the computer, and API keys, CC Switch, and deliberation data are not moved to the phone. A signed mobile session lasts at most 12 hours and never stores the raw pairing token in its cookie. The desktop reports active sessions and recent access, and can revoke all mobile sessions immediately. Restarting Council rotates the pairing token and invalidates old pairings. Mobile access stops when the computer sleeps, shuts down, or exits Council.
