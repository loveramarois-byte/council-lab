...
<h1 align="center">复杂问题，先看见分歧，再做决定。</h1>

<p align="center">
  Council Lab 是一个本地优先的 AI 审议工作台：让多个模型席位公开讨论同一个问题，保留不同意见、风险和取舍，最后由你确认是否形成答案。
</p>
...
普通聊天往往只给出一个顺滑的答案，却把分歧、前提和遗漏藏起来。Council 把这些过程公开化：

- **分开思考**：四个席位可以连续审议，也可以先独立作答再公开讨论。
- **保留异议**：席位必须回应前文；明确反对、未解决的问题和限制不会被总结成“大家都同意”。
- **人在回路中**：第四席结束后默认停下来，等你补充事实或确认是否继续。
- **留下记录**：完整公开发言、用量、结构化 DecisionBrief 和可移交报告都保存在本机。
...
- **传统文化不是科学结论**：该模式在浏览器本地生成固定版本的历法和排盘快照，研究传统规则及其解释；出生地不发送给模型，也不与高风险模式、自动总结或长期记忆混用。详见 [模式说明](docs/TRADITIONAL_CULTURE_MODE.md) 和 [第三方声明](NOTICE)。
