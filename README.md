...
- **资料空间与引用**：导入文字、网页、PDF、DOCX、Markdown、CSV、JSON 或 TXT，运行时固化资料快照并使用 `[S1]` 引用。
- **可移交报告**：审议完成后导出 Markdown 或单文件 HTML，保留问题、逐席发言、模型记录、用量和来源。
- **本地优先与密钥保护**：数据默认留在本机，API Key 交给系统凭据库保存。
- **可重复评测**：内置 12 个决策、事实核查、风险和规划案例，生成匿名盲评包；Mock 只验证流程，不形成效果结论。
