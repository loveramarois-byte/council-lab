...
传统文化模式不是“让一个模型断言命运”，而是把本地排盘、参考方向、流派比较、反证和限制放进同一条公开研究流程：

`本地计算快照 → 选择参考方向 → 四席独立研读 → 校历与流派比较 → 反证 → 用户确认`

- 本地服务使用锁定版本的规则引擎重新计算并签名快照；原始出生地文本不发送给模型，也不进入报告。
- 联网时间需要多个 HTTPS 时间来源达成一致；真太阳时校正会明确记录城市级坐标、民用时和修正值。
- 典籍只作为书名、主题和流派索引，不内置全文，不把索引伪装成原文引用。
- 最终答案先说白话结论，再解释计算字段、传统含义、流派分歧和反证边界。
- 传统解释不可科学验证，不提供正确率，也不能用于医疗、法律、投资、合规或生产决策。

完整边界见 [传统文化模式说明](docs/TRADITIONAL_CULTURE_MODE.md) 和 [第三方声明](NOTICE)。
...
当前版本为 `0.15.7`，适合个人研究、方案讨论和非约束性多视角决策辅助。Council 不执行外部网页搜索或代码沙箱核验；高风险模式可以记录证据、复核和责任声明，但不验证执照真伪，也不构成医疗器械、法律服务、投资顾问或合规认证。不要把模型输出直接用于高风险执行。
