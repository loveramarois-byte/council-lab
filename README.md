...
手机端是电脑端 Council 的远程界面：请求由电脑转发给模型，API Key、CC Switch 和审议数据不迁移到手机。手机获得最长 12 小时的签名会话，原始配对令牌不会写入 Cookie；电脑端可查看活动会话数量和最近访问时间，并随时撤销全部手机会话。每次重启 Council 都会轮换配对令牌，旧配对自动失效。电脑关机、休眠或退出 Council 后，手机端无法继续使用。
