...
- **本地可复现**：历法和排盘快照在浏览器本地生成并冻结版本；原始出生地文本不发送给模型或写入报告，只传递城市级解析和计算结果。
...
- **传统文化不是科学结论**：该模式在浏览器本地生成固定版本的历法和排盘快照，并可选择《穷通宝典》《三命通会》《滴天髓》《渊海子平》《千里命稿》《协纪辨方书》《果老星宗》《子平真诠》《神峰通考》等典籍研究方向；这些只是书名与主题索引，不内置全文或伪造引文。原始出生地文本留在本机且不进入报告，模型只收到城市级解析和计算结果；该模式也不与高风险模式、自动总结或长期记忆混用。HTTPS 多源时间是带短时服务端证明的秒级联网校时，不是专用 NTP 或天文台授时。详见 [模式说明](docs/TRADITIONAL_CULTURE_MODE.md) 和 [第三方声明](NOTICE)。
