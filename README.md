...
- **传统文化联合研判**：出生资料先在浏览器内由固定版本的 `lunar-javascript` 与 `iztro` 生成可校验快照，再由校历、辨典、参派、证伪四席独立初答；计算事实、传统解释、流派分歧和反证限制分开显示。
- **短任务精简路线**：快速档的短定义和确定性算术只调用一个讨论席与总结席，复杂任务仍保留完整四席。
- **全程可参与**：讨论中随时插话，用户输入会进入后续公开上下文。
- **最终确认点**：讨论席完成后默认等待你确认；你也可在创建时明确开启自动总结。高风险模式禁止自动总结。
- **高风险决策门禁**：可在创建时启用医疗、法律、投资、合规和生产事故模式；关键事实必须绑定来源、时间、有效期和独立核验，报告需逐域专业复核后再由另一名已配置复核者批准。
...
首页可切换到“传统文化”。该模式要求公历出生日期、出生时间、时间精度和排盘参数；出生地可选、仅保存在本地且不会发送给模型，不收集姓名。排盘在本机浏览器完成，不调用第三方命理 API。只有用户明确勾选同意后，冻结快照中的排盘字段和必要出生参数才会进入本次已配置的模型席位上下文。

当前计算口径固定为 `Asia/Shanghai` 民用时，不做真太阳时校正。结果保留 `lunar-javascript@1.7.7`、`iztro@2.5.8` 的版本、来源与 SHA-256，并展示四柱、历法字段和紫微十二宫。`bazi-skill` 仅用于参考交互与解释边界；`ziwei-doushu` 本身依赖 `iztro` 和 `lunar-javascript`，因此不会被描述成第三、第四套独立计算引擎。

该模式研究的是传统文化规则及其解释，不证明命理预测具有科学有效性。它强制采用四席独立初答，禁止与高风险模式、自动总结、历史决策记忆和专业输出契约混用；传统解释不会进入 DecisionBrief、可验证主张、决策回访或长期记忆。包含医疗、用药、法律、投资、合规或生产操作意图的问题会由服务端拒绝。

完整设计、隐私边界和第三方来源见 [传统文化联合研判说明](docs/TRADITIONAL_CULTURE_MODE.md) 与 [第三方声明](NOTICE)。

### 高风险模式

首页的“高风险决策支持”开关必须在创建 Run 时启用。服务端会先建立独立控制记录，再允许模型任务开始。运行期间先填写领域关键事实，再逐项追加来源、版本、时间、有效期和内容哈希；授权复核人必须独立核验每项证据。证据完整后才能形成非约束性报告，报告必须由匹配领域的专业角色逐域复核，最后再由另一名授权主体审批。过期、冲突、医疗紧急红旗、角色错配或专业复核缺失都会失败关闭。

专业角色是复核人的责任声明：系统会验证服务端复核密钥、领域角色白名单、证据快照和报告哈希，但不会联网核实执照真伪。即使状态为已完成，系统也不会执行开药、交易、法律文件提交、合规放行或生产变更。

控制面接受的角色标识包括：医疗 `physician` / `registered_nurse` / `pharmacist` / `clinical_specialist`，法律 `lawyer` / `attorney` / `legal_counsel` / `compliance_counsel`，投资 `licensed_adviser` / `investment_adviser` / `portfolio_manager` / `risk_officer`，合规 `compliance_officer` / `risk_officer` / `internal_auditor` / `compliance_counsel`。生产事故使用 `incident_commander` / `site_reliability_engineer` / `security_officer`。
...
当前版本为 `0.14.0`，适合个人研究、方案讨论和非约束性多视角决策辅助。高风险模式能记录证据核验和专业责任声明，但不验证执照真伪，也不构成医疗器械、法律服务、投资顾问或合规认证；请勿将输出直接用于高风险执行。
