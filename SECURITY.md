...
高风险模式是本地单用户应用中的 P0 控制面，不是医疗器械、法律服务、投资顾问、合规认证或生产自动化系统。它保存关键事实、证据元数据、独立核验记录、专业责任声明和非约束性报告，不执行交易、文件提交、药物变更、生产命令或其他外部副作用。证据核验由授权复核人负责，系统不会联网核实来源内容或执照真伪。

所有高风险 mutation 要求 `X-Council-Actor`，请求者身份与控制记录绑定。审批和风险覆盖还要求服务端环境变量 `COUNCIL_HIGH_RISK_REVIEWERS` 中配置的独立复核者 ID 与 secret；请求者默认不能自批。actor ID 仅用于本地归属和审计，不是密码学登录身份。手机配对令牌与会话不授予复核权限，UI 中显示按钮也不构成服务端授权。

复核 secret 不写入 SQLite、前端持久存储或审计 metadata。不要在命令历史、截图、聊天或报告中暴露它；优先通过受保护的启动环境注入，并在怀疑泄露时轮换。审批绑定 Run、动作类型、动作参数 SHA-256 和报告 SHA-256，具有到期、拒绝、撤销和一次性消费状态。SHA-256 只用于完整性绑定，不证明报告真实、正确或由专业人员撰写。

schema v5 的高风险审计表和 schema v12 的证据、证据核验、专业复核表均使用触发器拒绝 UPDATE/DELETE，普通 Run 删除接口也不能删除这些记录。这能防止应用常规路径篡改，不防御已取得本机账户和数据库写权限、能够移除触发器或替换整个数据库的攻击者。备份包含高风险事实、证据、专业声明、审批和审计数据；保护边界与主数据库相同。降级到旧版本必须恢复迁移前备份，不能让旧程序直接写更高版本数据库。
