...
高风险模式是本地单用户应用中的 P0 控制面，不是医疗器械、法律服务、投资顾问、合规认证或生产自动化系统。它只保存非约束性报告，不执行交易、文件提交、药物变更、生产命令或其他外部副作用，也没有联网事实/引用核验能力。
...
schema v5 的高风险审计表使用触发器拒绝 UPDATE/DELETE，普通 Run 删除接口也不能删除这些事件。这能防止应用常规路径篡改，不防御已取得本机账户和数据库写权限、能够移除触发器或替换整个数据库的攻击者。备份包含高风险事实、审批和审计数据；保护边界与主数据库相同。降级到旧版本必须恢复迁移前备份，不能让旧程序直接写 v5 数据库。
