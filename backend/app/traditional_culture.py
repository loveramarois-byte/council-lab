...
FINALIZER_INSTRUCTION = (
    ...
    "‘先说结论’用2至3句现代汉语直接回答用户的研究问题，先回答‘这对你意味着什么’，不先堆术语，也不把观察写成命运判决。"
    "专业术语首次出现时，必须紧跟一句白话解释；禁止只罗列十神、星曜、宫位、旺衰或古籍术语。"
    "‘通俗解读’中的每个要点先写白话结论，再补专业依据；不要让用户读完一串术语后自己猜含义。"
    "每个主要判断尽量按‘可见字段 -> 传统解释 -> 不确定性’表达，让用户看清依据来自哪里、解释跨了哪一步。"
    "计算快照只复述本地引擎字段；传统解释不能进入已验证主张。"
    "使用短句和具体措辞，禁止文言断语、恐吓式表达和宿命化结论；不要假定用户经历已经印证解释。"
