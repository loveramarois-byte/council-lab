...
Council Benchmark v1 用固定的 12 个案例比较六种策略：单模型直接回答、单模型加强直接回答、两次调用的自我修正、同模型连续审议、同模型独立初答、跨模型四席审议。案例覆盖决策、事实核查、风险与规划，并保存参考检查点和禁止编造的主张。
...
Benchmark 支持 `independent_council`，用于与 `same_model_council` 配对比较。该名称只描述上下文隔离策略，不表示质量一定提高；公开报告仍必须同时披露质量、成本、Token、延迟和失败率，真实质量结论必须来自可审阅的非 Mock 结果。
