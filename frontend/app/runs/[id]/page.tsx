...
export default function RunDetailPage() {
  ...
  const refresh = async (): Promise<void> => {
    ...
    try {
      ...
      } else { setDecisionBrief(null); setComparison(null); setClaims([]); setMemoryProposals([]); }
  ...
  return <div ref={pageRef} className={`council-page ${run.status === "completed" ? "is-completed" : ""} ${immersive ? "immersive" : ""}`}>
    ...
    {highRisk && highRiskOpen && <HighRiskPanel
      highRisk={highRisk} approval={highRiskApproval} audit={highRiskAudit}
      missingCriticalFacts={missingCriticalFacts} latestEvidenceByFact={latestEvidenceByFact}
      factDraft={factDraft} setFactDraft={setFactDraft} evidenceDraft={evidenceDraft} updateEvidenceDraft={updateEvidenceDraft}
      reportDraft={reportDraft} setReportDraft={setReportDraft}
      reviewerId={reviewerId} setReviewerId={setReviewerId} reviewerKey={reviewerKey} setReviewerKey={setReviewerKey}
      reviewerRole={reviewerRole} setReviewerRole={setReviewerRole} reviewDomain={reviewDomain} setReviewDomain={setReviewDomain}
      professionalScope={professionalScope} setProfessionalScope={setProfessionalScope}
      professionalAttestation={professionalAttestation} setProfessionalAttestation={setProfessionalAttestation}
      approvalReason={approvalReason} setApprovalReason={setApprovalReason}
      busy={highRiskBusy} error={highRiskError} onClose={() => setHighRiskOpen(false)}
      onSaveFacts={saveHighRiskFacts} onAddEvidence={addHighRiskEvidence} onVerifyEvidence={verifyHighRiskEvidence}
