...
export default function RunsPage() {
  ...
  return <div className="page-wrap"><header className="topbar"><div><span className="top-kicker">工作台 / 记录</span><span className="top-title">历史审议</span></div><Link href="/" className="top-action"><span>新建</span><ArrowUpRight size={15} /></Link></header><div className="list-intro"><p className="eyebrow terracotta">ARCHIVE / 02</p><h1>留下判断的来路。</h1><p>每一次审议都保存问题、公开发言、席位模型、分歧和运行限制。</p></div><div className="list-toolbar"><label className="search-box"><Search size={16} /><input placeholder="搜索问题" value={query} onChange={(event) => setQuery(event.target.value)} /></label><label className="filter-box"><Filter size={15} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">全部状态</option><option value="queued">排队中</option><option value="running">进行中</option><option value="awaiting_final_input">等待确认</option><option value="completed">已完成</option><option value="cancelled">已取消</option><option value="failed">可恢复失败</option><option value="stopped">达到限制</option></select></label></div><div className="run-list">{filtered.length === 0 ? <div className="empty-state"><span className="empty-number">00</span><h2>还没有审议记录</h2><p>从一个具体问题开始，答案会在这里留下轨迹。</p><Link href="/" className="text-action">开始第一份审议 <ArrowUpRight size={14} /></Link></div> : filtered.map((run) => <article key={run.id} className="run-row"><div className="row-marker"><span className={`status-pill-dot status-${run.status}`} /></div><div className="row-main"><Link href={`/runs/${run.id}`} className="row-question">{run.question}</Link><div className="row-meta"><span>{run.mode === "standard" ? "标准" : run.mode === "quick" ? "快速" : "严谨"}</span><span>{new Date(run.created_at).toLocaleString("zh-CN", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}</span><span>{activeProviderCount(run)} 个 Provider</span></div></div><div className="row-status">{runStatusLabel(run.status)}<small>{run.final_decision ? "未外部核验 · " : ""}{run.usage.model_calls} 次调用 · {(run.usage.input_tokens + run.usage.output_tokens).toLocaleString()} Token</small></div><button className="icon-button row-delete" onClick={() => remove(run.id)} title="删除记录"><Trash2 size={15} /></button><ArrowUpRight className="row-arrow" size={17} /></article>)}</div></div>;
}
