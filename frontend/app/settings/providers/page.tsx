...
export default function ProvidersSettingsPage() {
  ...
  return <div className="page-wrap settings-page provider-settings-page">
    ...
    <div className="settings-layout provider-settings-layout">
      <aside className="settings-nav"><p className="eyebrow">设置</p>{[["providers", "模型供应商", Server], ["agents", "角色分配", Link2], ["budget", "预算与限制", FlaskConical], ["privacy", "数据与隐私", KeyRound], ["appearance", "外观", Globe2]].map(([id, label, Icon]) => <a key={id as string} className={`settings-nav-link ${id === "providers" ? "active" : ""}`} href={id === "providers" ? "/settings/providers" : `/settings/${id}`}><Icon size={15} />{label as string}<ChevronRight size={14} /></a>)}</aside>
            ...
            <div className="provider-form-compact">
              ...
              {showAdvanced && <div className="advanced-provider-fields">
                ...
                <div className="form-two">
                  ...
                  <label className="field"><span>测试档位</span><select value={current.reasoning_effort || "high"} disabled={current.id === "mock"} onChange={(event) => updateCurrent({ reasoning_effort: event.target.value as Provider["reasoning_effort"] })}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="xhigh">XHigh</option><option value="max">Max</option><option value="ultra">Ultra</option></select></label>
                </div>
              </div>}
            </div>
