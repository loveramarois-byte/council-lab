...
import { resolveTraditionalLocation } from "../lib/traditional-locations";
...
export function TraditionalCultureForm({ profile, consent, onProfileChange, onConsentChange }: Props) {
  ...
  const resolvedLocation = resolveTraditionalLocation(profile.birth_place);
  ...
  return <section className="culture-profile" aria-labelledby="culture-profile-title">
    <header>
      ...
      <div><strong id="culture-profile-title">本地排盘资料</strong><small>公历 · 联网校时 · 出生地可校正真太阳时</small></div>
    ...
    <div className="culture-fields">
      ...
      <label className="culture-place"><span>出生地 <small>城市级识别</small></span><input aria-label="出生地" type="text" maxLength={120} autoComplete="off" placeholder="例如：山东青岛" value={profile.birth_place} onChange={(event) => { const value = event.target.value; const location = resolveTraditionalLocation(value); onProfileChange({ ...profile, birth_place: value, true_solar_time_applied: Boolean(location) }); }} /></label>
    ...
    <div className={`culture-solar-control ${resolvedLocation ? "resolved" : ""}`}>
      <span>{resolvedLocation ? `已识别 ${resolvedLocation.name} · ${resolvedLocation.longitude.toFixed(4)}°E` : profile.birth_place ? "未识别该城市，暂按民用时排盘" : "填写出生地后自动识别城市级经度"}</span>
      <label><input type="checkbox" aria-label="应用真太阳时校正" disabled={!resolvedLocation} checked={Boolean(resolvedLocation && profile.true_solar_time_applied)} onChange={(event) => update("true_solar_time_applied", event.target.checked)} />应用真太阳时校正</label>
    </div>
    ...
    <label className={`culture-consent ${consent ? "accepted" : ""}`}><input type="checkbox" checked={consent} onChange={(event) => onConsentChange(event.target.checked)} /><LockKeyhole size={14} /><span><strong>我同意将排盘字段和必要出生参数发送给本次已配置的五个模型席位</strong><small>仅发送城市级地点与本地计算的时间校正结果，不发送更细地址；不调用第三方命理 API。未勾选不会创建 Run。</small></span></label>
  </section>;
}
