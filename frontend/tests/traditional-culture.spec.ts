...
async function mockHome(page: import("@playwright/test").Page) {
  ...
  await page.route("**/api/traditional/snapshot", (route) => {
    const profile = route.request().postDataJSON();
    const value = snapshot();
    value.profile = { ...value.profile, ...profile, birth_place_normalized: "青岛", birth_latitude: 36.0671, birth_longitude: 120.3826, birth_place_source: "offline_city_catalog", true_solar_time_applied: true };
    value.snapshot_proof = `v1.${"a".repeat(64)}`;
...
test("renders provenance and boundaries on a mobile result without decision assets", async ({ page }) => {
  ...
  await expect(card).toContainText("《滴天髓》");
  await expect(card).toContainText("《周易》");
  ...
  await expect(page.getByText(cultureSnapshot.timing_facts.time_proof, { exact: false })).toHaveCount(0);
  await expect(page.getByText(cultureSnapshot.snapshot_proof, { exact: false })).toHaveCount(0);
